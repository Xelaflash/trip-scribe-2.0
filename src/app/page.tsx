...
const Home = () => {
  return (
    <>
      <section className="-mt-headerHeight min-h-[78vh] bg-[linear-gradient(90deg,hsl(var(--foreground)/0.9),hsl(var(--primary)/0.58),hsl(var(--secondary)/0.18)),url('/homeBg.jpg')] bg-cover bg-center">
        <div className="flex min-h-[78vh] items-center px-viewportPadding pt-[calc(var(--header-height)+3rem)] pb-16">
          <div className="w-full max-w-176 text-hero-foreground">
            <p className="m-0 text-xs font-extrabold tracking-[0.12em] text-secondary uppercase">
              Travel planning, written clearly
            </p>
            <h1 className="mt-3 mb-0 text-[3.3rem] leading-[0.92] text-hero-foreground min-[861px]:text-[clamp(3rem,8vw,6.5rem)]">
              {SITE_TITLE}
            </h1>
            <p className="mt-5 max-w-xl text-xl text-hero-foreground/90">
      ...
      <section className="border-y border-border bg-surface px-viewportPadding py-16">
        <div className="mx-auto grid w-full max-w-280 gap-8">
          <div>
            ...
            <h2 className="mt-2 mb-0 max-w-2xl text-[clamp(2rem,4vw,3rem)] leading-none text-foreground">
          ...
          <div className="grid grid-cols-1 gap-4 min-[861px]:grid-cols-4">
            {highlights.map((item) => {
              ...
              return (
                <article
                  key={item.title}
                  className="min-h-56 rounded-lg border border-border bg-card p-5 text-card-foreground shadow-elevationLow"
                >
                  <div className="flex size-11 items-center justify-center rounded-md bg-mint-100 text-emerald-800 dark:bg-primary/15 dark:text-primary">
